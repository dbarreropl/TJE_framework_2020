...
void Scene::loadScene() {
	...
	for (int i = 0; i < data_lines.size(); i++) {
		...
		Entity* entity = new EntityMesh(("data/biglib/WesternPack_renamed/All/"+data_spl[0]).c_str(),"data/biglib/WesternPack_renamed/texture.tga");
		
		//position
		std::vector <std::string> pos = split(data_spl[1], ',');
		//Camera* cam = (Camera*)Scene::instance->entities[0];		
		//Vector3 posc = cam->unproject(Vector3(std::stof(pos[0]), std::stof(pos[1]), std::stof(pos[2])),800,600);
		entity->model.setTranslation(-std::stof(pos[0]), std::stof(pos[1]), std::stof(pos[2]));
		//entity->model.setTranslation(posc.x,posc.y,posc.z);
